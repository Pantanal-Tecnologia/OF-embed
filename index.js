class OfEmbed {
  ...
  addModal(src, target) {
    ...
    if (!target) {
      ...
      modalBackground.addEventListener('click', () =>
        this.closeModal(modalBackground)
      )
  ...
  validacaoFornecedor(
    ...
    const src = `/#/API/${
      this.sistema
    }/validacaoFornecedor/${layout}/${transacoesJoined}/${this.token}/${btoa(
      docAddJson
    )}`
    let modal = null
    if (params.modal) {
    ...
    window.addEventListener(
      ...
      (event) => {
        console.log(
          ...
          event.origin,
          this.endPointFront,
          this.endPointFront == event.origin
        )
  ...
  validacaoFornecedorLote(
    ...
    const src = `/#/API/${
      this.sistema
    }/validacaoFornecedorLote/${layout}/${transacoesJoined}/${this.token}/${btoa(
      docAddJson
    )}`
    let modal = null
    if (params.modal) {
      ...
      fetch(this.endPointBack + '/loadRelatorioTransacoesDataAPI', init)
        .then(async function (res) {
          ...
          data?.transacoes?.forEach((element) => {
            ...
            result[element.produtoId][element.id] = element.nfId
              ? element.docs
              : false
          })
          resolve(result)
